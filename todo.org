...
** TODO switch to url-namespaced UUIDv5 and map
...
** TODO send uuid to front-end inside html fragments
inside hidden field:
#+begin_src html
<div>
...
** TODO use datastar edit-row
pseudocode:
- needs data-bind (?)
- edit+delete buttons should only appear on hover
#+begin_src html
<div>
    <input type="hidden" name="id" value="{{.UUID}}">
    <img src="{{.Favicon}}" class="..." alt="icon">
    <a href="{{.URL}}" target="_blank" class="...">{{.Title}}</a>
    <span class="...">{{.Category}}</span>
    <button data-on:click="@get('/api/bookmarks/edit')">🖉</button>
    <button data-on:click="@delete('/api/bookmarks/delete)">🗑</button>
</div>
#+end_src
...
<div>
    ...
    <input type="text" data-bind:url>
    <button data-on:click="@get('...')">🗙</button>
    <button data-on:click="@patch('/api/bookmarks/edit')">💾</button>
</div>
#+end_src
