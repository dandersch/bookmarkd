* TODO dev: add tests
* TODO extension: improve "add-bookmark" js bookmarklet
** TODO fix CORS problem that occurs on some sites (if possible)
Current understanding of the problem:
- js bookmarklets run in the context of the current page's js execution
  environment (and not on a separate thread in its own environment).
- this means we have to respect the site's Content Security Policies (CSP)
- some sites block any external script or fetch call from running
- confirmed: GitHub

** TODO auto-generate bookmarklet
- on index.html (no access auth data?)
- OR inside options.html
* TODO syncthing-able bookmarks.json/bookmarks.html
- needs file watcher in go backend to check for changes
- maybe switch to NETSCAPE-Bookmark-file-1 (html)
* TODO feat: simple filestash
* TODO feat: tags
- bookmarks can have several tags
- tags can be searched for
* TODO feat: auto-tag by url
- user-editable list of urls to auto-tag
- comes with a list 
- can be turned on/off
- example:
#+begin_src txt
youtube.com                      video
substack.com                     blog
food.com                         recipe
#+end_src
** TODO auto-tag specialization
- ability to add more specific urls of the same domain for auto-tagging
- apparently known as Longest Prefix Match (LPM) or Most Specific Match
- example (3rd pattern implies tag "issue", though 1st+2nd pattern also match):
#+begin_src txt
github.com                       software
github.com/.*/[a-Z]*             repo
github.com/.*/.*/issues/[0-9]*   issue 
#+end_src

* TODO feat: rss/opml integration
** TODO auto-find rss feed
- maybe use rsshub's json db (~800kB)
- https://rsshub.domain.xyz/api/radar/rules
** TODO opml as a shareable html blogroll
- [[https://web.archive.org/web/20251223065249/https://kmaasrud.com/blog/opml-is-underrated.html][opml is underrated]]
- [[https://web.archive.org/web/20251223065250/https://kmaasrud.com/blogroll.xml][example of opml as html]]
* TODO feat: detect changes of bookmark
** TODO periodically pull html and check if it has changed
- maybe provide a (visual) diff
** TODO add sort-by "last changed"
* TODO feat: snapshot webpage
** TODO take screenshot of webpage
** TODO save pdf of webpage
* TODO feat: annotate webpages
- integrated within sidebar
** DONE taking notes via simple textfield
CLOSED: [2026-01-21 Wed 03:27]
- one text box ready for every bookmark
** TODO sync annotation scroll with page scroll
- sidebar scroll needs to be able to sync with webpage
- try to anchor a notes position to a position on the webpage
- problem: mapping depends on zoom, screen dimensions, etc.
- maybe we can sync zoom level too
** TODO look into "real" web annotation
- challenge: anchoring notes so that they survive markup changes
  + see https://web.hypothes.is/blog/fuzzy-anchoring/
  + maybe integrate with "detect changes" feature somehow?
- see [[https://www.w3.org/TR/annotation-model/][w3c web annotation standard]]
** TODO auto-bookmark a site somewhere when user starts "annotating"
- reasoning: if you want notes for a site, you probably also want to remember
  that site, i.e. you do want it bookmarked somewhere
* TODO feat: notification integration
* TODO feat: wayback machine integration
- button that takes you to wayback 
- "save page now" button

* TODO feat: rss-hub integration
* TODO feat: pastebin
** TODO add button to send clipboard to bookmarkd
** TODO add button to copy into clipboard from bookmarkd
** TODO display clipboard on dashboard
** TODO shareable pastebin

* TODO feat: view page inside sidebar
- idea: mimick a split view by having a peek at page inside sidebar

* TODO look at similar projects
- [[https://news.ycombinator.com/item?id=47337607][sitespy - tool that watches webpages and exposes changes as RSS]]
- [[https://chromewebstore.google.com/detail/arcthing-window-tab-manag/bnfchihnnoljghnkdbpomeheeohjpnfo][arcthing]]: extension for arc-like tabs
- [[https://addons.mozilla.org/de/firefox/addon/sidebery/][sidebery]]: extension for tabs tree & bookmarks in sidebar
- [[arc.net][arc]]: browser
  + sidebar with bookmarks & tabs in one
  + workspaces w/ own color and containers
  + live preview widgets for certain websites
  + easel: whiteboard with site capture function
  + capture e.g. <div> of a page that will reload periodically
  + capture can be turned into live view, page can be maximized
- [[https://try.karakeep.app/dashboard/bookmarks][karakeep]]:
  + bookmark manager
  + save as pdf or offline archive
  + annotation feature (highlights)
  + thumbnail extraction (for youtube videos)
  + browser extension
- [[https://github.com/ndom91/briefkasten][briefkasten]]: bookmark manager + extension
- [[https://wallabag.org/][wallabag]]: archive & annotate webpages + extension
- [[https://github.com/awesome-selfhosted/awesome-selfhosted?tab=readme-ov-file#bookmarks-and-link-sharing][awesome selfhosted: bookmarks & link sharing]]
* DONE categories can have customizable colors
CLOSED: [2026-02-21 Sat 16:14]
* DONE better styling
CLOSED: [2026-02-21 Sat 16:14]
dashboard:
- no rounded corners, especially on cards
- "collapse-content" should have distinct background color, except for at the
  bottom: there should be padding/margin (unsure which one) to have distance to
  the next category, to make it visually more part of its category divider. The
  visual should be like folders opening/closing for categories (un-)collapsing.
- cards should be narrower but can be taller, 4 cards should fit in one row
- search bar should be bigger and centered, should take up most of the center space

* DONE feat: sort-by
CLOSED: [2026-02-11 Wed 12:07]
- when added
- title of bookmark
- url of bookmark
- last visited
* DONE [2/2] import existing bookmarks
CLOSED: [2026-01-24 Sat 14:29]
** DONE via a bookmarks.html / bookmarks.json file
CLOSED: [2026-01-16 Fri 17:50]
** DONE directly from browser using a api+permission
CLOSED: [2026-01-24 Sat 14:29]
* DONE dev: simple docker container
CLOSED: [2026-01-21 Wed 02:54]
- bookmarks db inside data/ folder
* DONE [2/2] feat: last visited
CLOSED: [2026-01-21 Wed 02:56]
** DONE log timestamp when user visits a bookmark in extension
CLOSED: [2026-01-20 Tue 10:29]
** DONE log timestamp when user visits a bookmark on dashboard
CLOSED: [2026-01-20 Tue 10:29]
* DONE [4/4] extension: better sidebar ui
CLOSED: [2026-01-18 Sun 20:50]
** DONE smaller bookmarks list elements
CLOSED: [2026-01-06 Tue 23:33]
** DONE preview current bookmark
CLOSED: [2026-01-07 Wed 04:03]
** DONE switch to icon-only view when sidebar becomes too narrow
CLOSED: [2026-01-09 Fri 20:18]
- should show only favicons of bookmarks in this view
- preview current page bar not needed in this view
- "Bookmark Page" button should become single bookmark symbol/icon (🔖/📌/+)
- Header would need to go from left-right arranging settings/bookmark/dashboad
  buttons to arranging them top-down
- No more hover to show edit/delete buttons
- ideally sidebar snaps to this minimum-width once it reaches a certain small width

notes:
- minimum-width for both chrome & firefox is too wide by default
- can only be achieved with custom userChrome.css
** DONE better colors
CLOSED: [2026-01-18 Sun 20:50]
* DONE [3/3] dashboard: better dashboard ui
CLOSED: [2026-01-18 Sun 20:50]
** DONE card view
CLOSED: [2026-01-09 Fri 20:50]
** DONE category divider
CLOSED: [2026-01-16 Fri 17:49]
** DONE better colors
CLOSED: [2026-01-18 Sun 20:50]
* DONE extension: indicate if current page is already bookmarked
CLOSED: [2026-01-18 Sun 21:22]
- +might require more permissions and a background worker+
- +indicate by changing the icon of the extension+
* DONE feat: search option [2/2]
CLOSED: [2026-01-18 Sun 20:51]
** DONE add to dashboard
CLOSED: [2026-01-09 Fri 20:50]
** DONE add to sidebar
CLOSED: [2026-01-18 Sun 20:51]
* DONE dev: start using tailwind compiler
CLOSED: [2026-01-17 Sat 15:57]
* DONE options page accessible both dashboard and sidebar
CLOSED: [2026-01-16 Fri 17:49]
* DONE dark/light mode
CLOSED: [2026-01-16 Fri 17:50]
* DONE improve category UI/UX
CLOSED: [2026-01-16 Fri 17:50]
** DONE add divider between categories
CLOSED: [2026-01-15 Thu 04:48]
** DONE collapsible categories
CLOSED: [2026-01-15 Thu 04:48]
see [[https://daisyui.com/components/collapse/][Tailwind Collapse Component – daisyUI]] 
* DONE extension: switch to sidebar/sidepanel view
CLOSED: [2026-01-09 Fri 20:19]
** DONE chrome support
CLOSED: [2026-01-09 Fri 20:18]
- uses sidePanel api. manifest.json:
  #+begin_src json
  "permissions": [
    "sidePanel",
    ...
  ],
  "side_panel": {
    "default_path": "popup.html"
  },
  #+end_src

- requires background worker:
  #+begin_src javascript
  chrome.action.onClicked.addListener((tab) => {
    chrome.sidePanel.setOptions({
      tabId: tab.id,
      path: 'popup.html',
      enabled: true
    });
    chrome.sidePanel.setPanelBehavior({
      openPanelOnActionClick: true // toggles sidebar when clicking extension icon
    });
    chrome.sidePanel.open({ tabId: tab.id });
  });
  #+end_src

** DONE firefox support
CLOSED: [2026-01-09 Fri 20:19]
- firefox uses sidebar_action in manifest.json:
  #+begin_src json
  "sidebar_action": {
    "default_icon": "icon.png",
    "default_title": "My sidebar",
    "default_panel": "popup.html",
    "open_at_install": true
  },
  #+end_src

- requires background worker:
  #+begin_src javascript
  chrome.action.onClicked.addListener((tab) => {
    browser.sidebarAction.toggle();
  });
  #+end_src

- set sidebar.revamp to false to get (better) old ui

- to be able to get smaller min/max-width sidebar:
  + open profile directory (about:support -> Profile Directory)
  + create folder "chrome"
  + create "userChrome.css" inside "chrome", add:
  #+begin_src css
  /* remove maximum/minimum width restriction of sidebar */
  #sidebar-box {
    max-width: none !important;
    min-width: 0px !important;
  }
  #+end_src

* DONE extension: current bookmark preview breaks                       :bug:
CLOSED: [2026-01-09 Fri 19:55]
- stops working sometimes
- doesnt work for pinned pages (?)
- doesnt work when viewing a page opened in a new tab
- doesn't stay up to date when switching pages
- after saving, stays on "Saving..." button and doesnt remove "Saved!" popup
- janky in general

* DONE customizable settings for every bookmark
CLOSED: [2026-01-09 Fri 19:37]
** DONE switch to url-namespaced UUIDv5 and map
CLOSED: [2026-01-09 Fri 19:34]
example of how firefox exports a bookmark to json:
#+begin_src json
{
  "guid": "VNQYuxxt4jYL",
  "title": "namecheap",
  "index": 6,
  "dateAdded": 1651705172249000,
  "lastModified": 1756168838000000,
  "id": 20,
  "typeCode": 1,
  "iconUri": "https://www.namecheap.com/assets/img/nc-icon/favicon.ico",
  "type": "text/x-moz-place",
  "uri": "https://www.namecheap.com/myaccount/login/"
}
#+end_src

go code (uuid library was already included):
#+begin_src go
u := uuid.NewSHA1(uuid.NameSpaceURL, []byte(payload.URL))
#+end_src

also switch to using a map from uuid -> bookmark
#+begin_src go
// old
bookmarks []Bookmark

// new
bookmarks map[string]Bookmark
#+end_src

use following js code for generating uuids on the frontend if needed:
#+begin_src javascript
async function generateUUIDv5(url) {
    // Standard URL Namespace UUID: 6ba7b811-9dad-11d1-80b4-00c04fd430c8
    const NAMESPACE_URL = [
        0x6b, 0xa7, 0xb8, 0x11, 0x9d, 0xad, 0x11, 0xd1,
        0x80, 0xb4, 0x00, 0xc0, 0x4f, 0xd4, 0x30, 0xc8
    ];

    // 1. Convert URL to UTF-8 bytes
    const urlBytes = new TextEncoder().encode(url);

    // 2. Combine Namespace + URL bytes
    const combined = new Uint8Array(NAMESPACE_URL.length + urlBytes.length);
    combined.set(NAMESPACE_URL);
    combined.set(urlBytes, NAMESPACE_URL.length);

    // 3. SHA-1 Hash
    const hashBuffer = await crypto.subtle.digest('SHA-1', combined);
    const hashBytes = new Uint8Array(hashBuffer);

    // 4. Set Version (5) and Variant (RFC4122) bits
    hashBytes[6] = (hashBytes[6] & 0x0f) | 0x50; // Version 5
    hashBytes[8] = (hashBytes[8] & 0x3f) | 0x80; // Variant 10xxxxxx

    // 5. Convert to Hex String
    const hex = Array.from(hashBytes)
        .map(b => b.toString(16).padStart(2, '0'))
        .join('');

    // Format as 8-4-4-4-12
    return `${hex.slice(0, 8)}-${hex.slice(8, 12)}-${hex.slice(12, 16)}-${hex.slice(16, 20)}-${hex.slice(20, 32)}`;
}
#+end_src
  
** DONE send uuid to front-end inside html fragments
CLOSED: [2026-01-09 Fri 19:34]
inside hidden field:
#+begin_src html
<div>
    <input type="hidden" name="id" value="{{.UUID}}">
    <img src="{{.Favicon}}" class="..." alt="icon">
    <a href="{{.URL}}" target="_blank" class="...">{{.Title}}</a>
    <span class="...">{{.Category}}</span>
</div>
#+end_src

** DONE add edit and delete buttons + endpoints
CLOSED: [2026-01-09 Fri 19:37]
Backend (main.go):                                                                                                                 
- DELETE ~/api/bookmarks/:id~: removes bookmark, returns 204
- PATCH ~/api/bookmarks/:id~: updates title, returns 200

Frontend (components.js):
- <bookmark-item> shows ✎ (edit) and 🗑 (delete) buttons on hover
- Edit: inline text input with ✓ (save) and ✕ (cancel) buttons
- Enter saves, Escape cancels
- Gets server-url and auth-header from parent <bookmark-list>

* DONE extension: bookmarking current page doesnt work after switching tabs
CLOSED: [2026-01-07 Wed 03:41]
see https://stackoverflow.com/questions/77276350/chrome-extension-how-to-show-the-active-tab-url-with-updates-when-it-changes

#+begin_src js
// Get current tab info
const [tab] = await chrome.tabs.query({ active: true, currentWindow: true });
#+end_src
[[file:~/dev/bookmarkd/extension/popup.js::const \[tab\] = await chrome.tabs.query({ active: true, currentWindow: true });]]
* DONE better one-click add-bookmark UX
CLOSED: [2026-01-04 Sun 00:37]
possibilities:
- +separate extension that allows one-click bookmarking?+
- +add contextmenu entry for two-click bookmarking+
- javascript bookmarklet to add bookmark
* DONE test running on vps behind reverse proxy w/ basic auth
CLOSED: [2026-01-04 Sun 00:35]
solution:
- allow OPTIONS requests at PathPrefix(`/api`) to pass-through without
  basic-auth
- needed so browser can send its preflight requests (no way to set it to use
  basic-auth possible it seems)
- service set to run at docker bridge ip via .env
